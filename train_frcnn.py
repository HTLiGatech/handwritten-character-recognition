...
try:
	with open("image4500.pkl", "rb") as f:
	    example_list = cPickle.load(f)
except IOError:
	train_imgs = sorted(train_imgs)
	sample_selector = data_generators.SampleSelector(classes_count)
	random.shuffle(train_imgs)

	for train_img in  train_imgs:
		print('Elapsed time: {}'.format(time.time() - start_time))
		example_list.append(data_generators.get_anchor_gt(train_img, sample_selector, C, K.image_dim_ordering(), mode='train'))

	cPickle.dump(example_list, open('image4500.pkl', 'wb'), -1)
else:
...
# jobs = [(input, job_server.submit(data_generators.get_anchor_gt,(input,sample_selector, C, K.image_dim_ordering()), \
# 	(data_generators.SampleSelector,data_generators.data_augment.augment, data_generators.get_new_img_size, \
# 	data_generators.calc_rpn, ),("cv2","numpy",))) for input in train_imgs]
# for input, job in jobs:
#     example_list.append(job())
#     print('Elapsed time: {}'.format(time.time() - start_time))
...
# example_list = [data_gen_train.next() for _ in range(len(train_imgs))]
...
while True:
	try:
		...
		# print('bf, Elapsed time: {}'.format(time.time() - start_time))
		# X, Y, img_data = data_gen_train.next()
		# print('af, Elapsed time: {}'.format(time.time() - start_time))
		idx = random.randint(0, len(example_list)-1)
		X, Y, img_data = example_list[idx]
		...
		print('Elapsed time: {}'.format(time.time() - start_time))
		print('epoch_length: {}'.format(len(rpn_accuracy_rpn_monitor)))
